...
async function home(request: Request) {
  ...
  const text = coinsKeys.map(c => `${coins[c].name} (${c}) : ${coins[c].quote.EUR.percent_change_24h > 0 ? ':green_circle:' : ':red_circle:'}${Math.round(coins[c].quote.EUR.price * 100) / 100}€ (${Math.round(coins[c].quote.EUR.percent_change_24h * 100) / 100} en 24h)`).join('\n')
